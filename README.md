...
API 키는 두 가지 방법으로 주입할 수 있습니다.

1. **MCP 클라이언트 설정의 `env` 항목** (권장) — 아래 설치 예시 참고
2. **서버 실행 디렉터리의 `.env` 파일** — 서버 시작 시 자동으로 읽으며, 이미 설정된 환경변수를 덮어쓰지 않습니다
...
> 참고: `npx`로 실행할 때의 작업 디렉터리는 MCP 클라이언트가 정하므로, `.env` 방식은 로컬 clone 실행 시에 적합합니다. 클라이언트 설정의 `env` 주입이 가장 확실합니다.
